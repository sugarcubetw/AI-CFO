...
export default function FinancePage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "add" | "ledger" | "income" | "payments" | "query" | "monthly" | "settings">("overview");
  ...
  return <main className={`finance-app finance-tab-${activeTab}`}>
    <header className="finance-header"><div><p>方糖民宿</p><h1>支出記帳</h1></div><div className="finance-header-actions"><a className="finance-back-link" href="/">← 工作台</a><span className={online ? "online" : "offline"}>{online ? "已連線" : "離線"}</span></div></header>
    <nav className="finance-tabs" aria-label="財務功能"><button type="button" className={activeTab === "overview" ? "active" : ""} onClick={() => setActiveTab("overview")}>總覽</button><button type="button" className={activeTab === "add" ? "active" : ""} onClick={() => setActiveTab("add")}>支出</button><button type="button" className={activeTab === "income" ? "active" : ""} onClick={() => setActiveTab("income")}>收入</button><button type="button" className={activeTab === "payments" ? "active" : ""} onClick={() => setActiveTab("payments")}>待收</button><button type="button" className={activeTab === "query" ? "active" : ""} onClick={() => setActiveTab("query")}>查詢</button><button type="button" className={activeTab === "monthly" ? "active" : ""} onClick={() => setActiveTab("monthly")}>月結</button><button type="button" className={activeTab === "settings" ? "active" : ""} onClick={() => setActiveTab("settings")}>設定</button></nav>
    ...
    {(activeTab === "income" || activeTab === "payments" || activeTab === "settings") && <section className="finance-placeholder"><h2>{activeTab === "income" ? "收入" : activeTab === "payments" ? "付款與待收款" : "財務設定"}</h2><p>{activeTab === "income" ? "收入由營運模組訂單自動讀取，不在此手動新增。" : activeTab === "payments" ? "依有效訂單查看已收款與待收尾款。" : "管理支出分類、付款方式、AI 分類與月結設定。"}</p></section>}
  </main>;
}
