...
export default function IncomeDashboard() {
  ...
  return <section className="income-dashboard" aria-label="收入 Dashboard">
    ...
    {!loading && data && view === "calendar" && <div className="income-panel"><h3>B｜住房日曆</h3><p className="income-muted">住房日為入住日起至退房日前一天，退房日不計入。</p><div className="income-date-grid">{data.daily.map((item) => <button type="button" key={item.date} className={selectedDate === item.date ? "selected" : ""} onClick={() => setSelectedDate(item.date)}><b>{item.date.slice(5).replace("-", "/")}</b><span>{item.rooms} 房</span><strong>{money(item.total)}</strong><em>{item.unrealized > 0 ? "未實現" : "已實現"}</em></button>)}</div>{selectedDay && <div className="income-selected-detail"><div className="income-selected"><b>{selectedDay.date} 住房清單</b><span>{selectedDay.rooms} 房 · 已實現 {money(selectedDay.realized)} · 未實現 {money(selectedDay.unrealized)}</span></div><div className="income-stay-list">{selectedDay.stays.map((stay) => <div className="income-stay-row" key={`${stay.id}-${stay.date}`}><div><strong>{stay.roomNumber ?? "未分房"}・{stay.guestName}</strong><small>{stay.roomType}・{stay.id}</small><small>住宿 {stay.arrivalDate} → {stay.departureDate}</small></div><b>{money(stay.amount)}<em>{stay.realized ? "已實現" : "未實現"}</em></b></div>)}</div></div>}</div>}
