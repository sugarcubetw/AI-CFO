...
type StayDetail = { date: string; id: string; guestName: string; roomNumber: string | null; roomType: string; arrivalDate: string; departureDate: string; amount: number; realized: boolean };
type DailyIncome = { date: string; realized: number; unrealized: number; total: number; rooms: number; stays: StayDetail[] };
type RoomIncome = { roomNumber: string; roomType: string; nights: number; realized: number; unrealized: number; total: number; stays: StayDetail[] };
type IncomeOrder = { id: string; sourceChannel: string; guestName: string; arrivalDate: string; departureDate: string; roomNumber: string | null; roomNumbers?: string[]; roomLabel?: string; roomType: string; allocation: number; realized: number; unrealized: number; nights: number; createdAt: string };
type IncomeData = {
...
export default function IncomeDashboard() {
  ...
  return <section className="income-dashboard" aria-label="收入 Dashboard">
    ...
    {!loading && data && view === "calendar" && <div className="income-panel"><h3>B｜住房日曆</h3><p className="income-muted">住房日為入住日起至退房日前一天，退房日不計入；多房訂單會逐房列出。</p><div className="income-date-grid">{data.daily.map((item) => <button type="button" key={item.date} className={selectedDate === item.date ? "selected" : ""} onClick={() => setSelectedDate(item.date)}><b>{item.date.slice(5).replace("-", "/")}</b><span>{item.rooms} 房</span><strong>{money(item.total)}</strong><em>{item.unrealized > 0 ? "未實現" : "已實現"}</em></button>)}</div>{selectedDay && <div className="income-selected-detail"><div className="income-selected"><b>{selectedDay.date} 住房清單</b><span>{selectedDay.rooms} 房 · 已實現 {money(selectedDay.realized)} · 未實現 {money(selectedDay.unrealized)}</span></div><div className="income-stay-list">{selectedDay.stays.map((stay) => <div className="income-stay-row" key={`${stay.id}-${stay.date}-${stay.roomNumber ?? "unassigned"}`}><div><strong>{stay.roomNumber ?? "未分房"}・{stay.guestName}</strong><small>{stay.roomType}・{stay.id}</small><small>住宿 {stay.arrivalDate} → {stay.departureDate}</small></div><b>{money(stay.amount)}<em>{stay.realized ? "已實現" : "未實現"}</em></b></div>)}</div></div>}</div>}
    {!loading && data && view === "rooms" && <div className="income-panel"><h3>C｜房間收益</h3><p className="income-muted">依住房日分配；點選房間查看每一晚的日期與訂單。</p><div className="income-room-list">{data.rooms.map((room) => <button type="button" key={room.roomNumber} className={selectedRoom === room.roomNumber ? "selected" : ""} onClick={() => setSelectedRoom(room.roomNumber)}><span><b>{room.roomNumber}</b>{room.roomType}</span><small>{room.nights} 晚</small><strong>{money(room.total)}</strong></button>)}</div>{selectedRoomData && <div className="income-selected-detail"><div className="income-selected"><b>{selectedRoomData.roomNumber} {selectedRoomData.roomType}</b><span>{selectedRoomData.nights} 晚 · 已實現 {money(selectedRoomData.realized)} · 未實現 {money(selectedRoomData.unrealized)}</span></div><div className="income-stay-list">{selectedRoomData.stays.map((stay) => <div className="income-stay-row" key={`${stay.id}-${stay.date}`}><div><strong>{stay.date}・{stay.guestName}</strong><small>{stay.id}・住宿 {stay.arrivalDate} → {stay.departureDate}</small></div><b>{money(stay.amount)}<em>{stay.realized ? "已實現" : "未實現"}</em></b></div>)}</div></div>}</div>}
