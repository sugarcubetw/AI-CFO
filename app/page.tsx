...
type Order = {
  ...
  roomNumbers?: string[]; roomLabel?: string; roomAllocations?: { roomNumber: string; allocatedAmount: number; allocationMethod: string }[];
...
const roomText = (order: Pick<Order, "roomNumber" | "roomLabel">) => order.roomLabel ?? order.roomNumber ?? "未分房";
...
export default function Home() {
  ...
  // A checked-in guest departing today leaves a room that operations needs to clean.
  // `orders` covers the current seven-day window, including stays that started earlier.
  const cleaningOrders = useMemo(() => orders
    .filter((order) => order.status === "checked_in" && order.departureDate === today)
    .sort((left, right) => (left.roomNumber ?? "").localeCompare(right.roomNumber ?? "") || left.guestName.localeCompare(right.guestName)), [orders]);
  ...
  async function updateOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const roomType = base?.roomTypes.find((item) => item.id === roomTypeId);
    const response = await fetch("/api/orders", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ id: editOrderId, action: "update", adults: Number(form.get("adults")), children: Number(form.get("children")), roomTypeId, roomNumber: roomType?.defaultRoomNumber ?? null, specialRequests: form.get("specialRequests") }),
    });
    const result = await response.json() as { error?: string };
    if (!response.ok) return setMessage(result.error ?? "修改訂單失敗");
    setEditOrderId(""); setMessage("✓ 訂單房號、人數與房客留言已更新");
    await Promise.all([loadTodayOrders(), loadOrders()]);
  }
  ...
  return <main className={`app-shell${view === "calendar" ? " calendar-active" : ""}`}>
    <header className="app-header"><div><h1>方糖營運工作台</h1><p>{today}・接待人員</p></div><div className="header-actions"><form className="header-nav-form" action="/finance" method="get"><button className="header-link" type="submit">財務</button></form><form className="header-nav-form" action="/settings" method="get"><button className="header-link" type="submit">設定</button></form></div></header>
    ...
    {view === "new-orders" && editingOrder && base && <OrderEditForm order={editingOrder} base={base} onSubmit={updateOrder} onCancel={() => setEditOrderId("")} />}
    {view === "new-orders" && newOrders.orders.length > 0 && <section className="screen new-order-summary-list"><h2>新訂單（依進入時間）</h2>{newOrders.orders.map((order) => <article className="order" key={`summary-${order.id}`}><div className="spread"><strong>{order.guestName}</strong><span>{roomText(order)}</span></div><p>入住日期：{order.arrivalDate}</p><p>訂單日期：{order.createdAt.slice(0, 10)}</p><p>房號：{roomText(order)}</p><button className="secondary compact" type="button" onClick={() => { setOrders((current) => current.some((item) => item.id === order.id) ? current : [...current, order]); setEditOrderId(order.id); markNewOrderRead(order.id); }}>修改人數／留言</button></article>)}</section>}
    {view === "new-orders" && !editingOrder && newOrders.orders.length > 0 && <section className="screen new-order-edit-tools"><h3>新訂單資料確認</h3><p className="privacy">可先補充成人、兒童人數與飲食注意事項，修改會寫回同一筆訂單。</p><div className="action-row">{newOrders.orders.map((order) => <button type="button" className="secondary compact" key={order.id} onClick={() => { setOrders((current) => current.some((item) => item.id === order.id) ? current : [...current, order]); setEditOrderId(order.id); markNewOrderRead(order.id); }}>{roomText(order)}・{order.guestName} 編輯</button>)}</div></section>}
      ...
      {days.map((day) => {
        ...
        return <button type="button" role="gridcell" key={day} className={`calendar-day${day === today ? " today" : ""}${day === selectedDate ? " selected" : ""}${mode === "month" && dayDate.getUTCMonth() !== activeMonth ? " outside" : ""}`} onClick={() => onSelectDate(day)} aria-label={`${day}，${dayOrders.length} 筆訂單`}>
          ...
          <span className="calendar-events" style={{ gridTemplateRows: `repeat(${laneCount}, minmax(52px, auto))` }}>{dayOrders.map((order) => {
            ...
            const roomLabel = startsToday ? roomText(order) : `續住 ${order.roomNumber ?? roomText(order)}${order.roomLabel && order.roomLabel !== order.roomNumber ? `／${order.roomLabel}` : ""}`;
            return <i className={`calendar-event stay-color-${assignment.color} ${segmentShape}`} style={{ gridRow: assignment.lane + 1 }} key={order.id} title={`${order.guestName}・${roomText(order)}・${startsToday ? "入住" : "續住"}`}><span className="calendar-event-room">{roomLabel}</span><span className="calendar-event-guest">{order.guestName}</span></i>;
          })}</span>
        </button>;
      })}
        </div>
