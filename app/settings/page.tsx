...
type AutomationJob = { id: string; name: string; description: string; scheduleType: "interval" | "daily" | "event"; intervalMinutes: number | null; timeOfDay: string | null; timezone: string; isEnabled: boolean; lastRunAt: string | null; lastStatus: string | null };
...
const actionTitle: Record<string, string> = {
  ...
  "automation.updated": "修改排程服務", "automation.disabled": "停用排程服務",
};
...
export default function SettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const settings = await settingsResponse.json() as { roomTypes: RoomType[]; options: Option[]; automationJobs: AutomationJob[] };
    setRooms(settings.roomTypes); setOptions(settings.options); setAutomationJobs(settings.automationJobs); setMeals(await mealsResponse.json() as Meal[]); setLogs(await logsResponse.json() as Log[]); setMessage("");
  }, []);
  ...
  async function saveAutomation(event: FormEvent<HTMLFormElement>, job: AutomationJob) {
    event.preventDefault(); const form = new FormData(event.currentTarget); const body = Object.fromEntries(form.entries());
    const response = await fetch("/api/settings", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...body, entity: "automationJob", id: job.id, isEnabled: form.get("isEnabled") === "on" }) });
    const result = await response.json() as { error?: string };
    setMessage(response.ok ? `已儲存「${job.name}」並記錄 Log` : result.error ?? "排程儲存失敗");
  ...
  return <main className="app-shell settings-shell">
    ...
    {tab === "base" && <section className="screen settings-content">
      ...
      <section className="automation-group"><div className="section-heading"><h2>排程與通知服務</h2><span>{automationJobs.filter((job) => job.isEnabled).length} 個啟用</span></div><p className="field-note automation-help">週期設定會提供給常駐 Agent 執行；事件觸發服務只需設定開關。時區固定為台北。</p>{automationJobs.map((job) => <AutomationForm key={job.id} job={job} onSubmit={saveAutomation}/>)}</section>
    </section>}
...
function AutomationForm({ job, onSubmit }: { job: AutomationJob; onSubmit: (event: FormEvent<HTMLFormElement>, job: AutomationJob) => void }) {
  const [scheduleType, setScheduleType] = useState(job.scheduleType);
  return <form className={`automation-card ${job.isEnabled ? "" : "inactive"}`} onSubmit={(event) => onSubmit(event, job)}><div className="spread"><div><strong>{job.name}</strong><p>{job.description}</p></div><label className="switch-label"><input name="isEnabled" type="checkbox" defaultChecked={job.isEnabled}/> 啟用</label></div><div className="automation-fields"><div><label>執行方式</label><select name="scheduleType" value={scheduleType} onChange={(event) => setScheduleType(event.target.value as AutomationJob["scheduleType"])}><option value="interval">固定週期</option><option value="daily">每日定時</option><option value="event">事件觸發</option></select></div>{scheduleType === "interval" && <div><label>每隔幾分鐘</label><input name="intervalMinutes" type="number" min="5" max="1440" defaultValue={job.intervalMinutes ?? 15}/></div>}{scheduleType === "daily" && <div><label>每日時間</label><input name="timeOfDay" type="time" defaultValue={job.timeOfDay ?? "06:00"}/></div>}{scheduleType === "event" && <div className="event-description">有事件時立即執行</div>}</div><div className="automation-footer"><small>{job.lastRunAt ? `上次執行：${new Date(job.lastRunAt).toLocaleString("zh-TW")}・${job.lastStatus ?? "完成"}` : "尚無執行記錄"}</small><button className="inline-button" type="submit">儲存排程</button></div></form>;
}
