...
`id`, `reservation_id`, `external_payment_id`, `payment_stage`, `status`, `amount`, `currency`, `paid_at`, `refunded_at`, `payment_method`, `source_version`

`payment_stage`：`deposit`, `balance`, `full`, `refund`, `forfeit`, `other`。

### guest_private_profiles

保存必要敏感資料，與一般營運查詢分離：`id`, `reservation_id`, `encrypted_name`, `encrypted_contact`, `retention_until`。預設不提供給 LLM。
