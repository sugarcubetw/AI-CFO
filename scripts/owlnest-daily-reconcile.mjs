...
async function run() {
  ...
  try {
    ...
    if (conflicts > 0) await notify({ title: "需要人工確認", lines: [`區間：${range.from}～${range.to}`, `匯入 ${result.received} 筆`, `欄位差異 ${result.changed ?? 0} 筆`, `匯出未出現 ${result.missingFromExport ?? 0} 筆`, `匯出重複 ${result.duplicateInExport ?? 0} 筆`, `解析錯誤 ${result.errors?.length ?? 0} 筆`, `資料警示 ${result.warnings?.length ?? 0} 項`] });
    else process.stdout.write(`OwlNest 核對完成，${result.received} 筆，無需人工確認。\n`);
  } finally {
    await context.close();
  }
}
